...
$s=mysql_query("select `cname`, `contact` from `customer` where `cust_id`=$cust_id");
...
<?php
$sql2=mysql_query("SELECT sum( amt ) AS total FROM `payment_history` WHERE `cust_id`=$cust_id ") or die(mysql_error());
...
$sql1=mysql_query("SELECT `amt` ,`date` FROM `payment_history` WHERE `cust_id` =$cust_id order by date") or die(mysql_error());
